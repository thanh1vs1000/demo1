...
//
    ...
    public function LogoutEmployer(){
        ...
        return redirect()->route('company.get.index2')->with('logout','Đẵng xuất thành công');
    }
...
//        $language = Language::all();
    ...
    public function postAddJob(AddJob $request){
        ...
        if($request->hasFile('images')){
            $configFileUpload   = config('file_upload');
            ...
            if(!in_array($fileType, $configFileUpload['job_extension_allow'])){
        ...
        $aryInsert = [
            ...
            'deadline_job'      => $request['deadline_job'],
            ...
            'email_get_cv'      => $request['email_get_cv'],
            'images'            => $urlAvatar,
            'status'            => 0,
            'slug_name'         => '',
        ];

        Job::insert($aryInsert);
    ...
    public function getEditJob($id){
        ...
        $profile = CompanyUser::all('id');
        $language = Language::all();
        return view('pages.jobs.edit-job',compact('language','jobs','zones','job_categories','profile'));
...
//        $jobs['id_language'] = $request['id_language'];
    ...
    public function getListUser(){
        ...
        $users      = User::all();
        $recruit    = Recruitment::all()->where('company_id',$company->id);

        $recruitment = [];
        foreach($recruit  as $rec){
            $recruitment[$rec['job_id']][] = $rec->toArray();
        }

        // return view('pages.list-user.list-user',['recruitment'=>$recruitment,'company'=>$company,'zones'=>$zones,'jobs'=>$jobs,'users'=>$users]);
        return view('pages.list-user.list-user',compact('recruitment', 'company', 'zones', 'jobs', 'users'));
    }
    ...
    public function getCompanypass(){
        $employer = Auth::guard('company_user')->user();
        return view('pages.employer.changepass-company',['employer'=>$employer]);
    }
   public function postCompanypass(Request $request){
       $employer = Auth::guard('company_user')->user();
       ...
       if ($validatedData->fails()) {
           return View('pages.employer.changepass-company',['employer'=>$employer])->withErrors($validatedData);
       }
       $employer = Auth::guard('company_user')->user();
       if(password_verify($request->password, $employer['password']) == false){
           $mess="Mật khẩu không trùng khớp";
           return view('pages.employer.changepass-company', compact('mess'),['employer'=>$employer]);
       }
       //mã hóa mật khẩu mới
       ...
       return redirect()->route('company.get.changepass')->with('success',"Đổi mật khẩu thành công !");
   }
...
public function getDeleteUser($id){
    ...
    $recruitment->delete();
    return redirect()->back()->with('deletedone','Xóa ứng viên thành công');
